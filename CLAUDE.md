...
**Enemies** (`src/entities/enemies.ts`) are one `Enemy` shape with a `kind`. Droids follow the track; one at a time they're promoted to command ships (track-following, shoot, lay photon mines), which eventually become free-flying death ships (chase the player, bounce off walls, lay vapor mines). Tankers (`LevelDef.tankers`, from level 5) crawl the track the other way, take `hp` hits (`Game.hitEnemy`), and launch ships at random through `EnemyWorld.spawn` (`Game.launch`, which enforces `maxHunters` and `maxShips` and adds to `waveSize`). `isShip` is everything but mines: the wave clears when none are left. Mines stay put but get pushed by moving walls.
